    ...
    <div class="p-2">
        ...
        <div class="flex">
            <div class="w-9/12 self-center">
                <h1 class="text-xl font-bold">Les bases</h1>
            </div>
                    ...
                    <div class=" bg-white p-4 z-100 w-1/2 mx-auto mt-24 lg:my-8 self-center " >
                        ...
                        <div>
                            <form action="" @submit.prevent="storing">
                                <div class="mt-4">
                                    <InputView  name="name" labeling="Nom de la base" v-model="form.name" typeInput="text" idInput="name" />
                                </div>
        ...
        <ListBaseView :listBase="base"/>
    </div>
</template>
<script>
    import store from "@/store/index"
    import ListBaseView from "@/components/BaseComponents/ListBaseView.vue"
    ...
    export default{
        ...
        mounted() {
            store.dispatch('base/index',this.$route.params.id).then((data)=>{
                data.data.base.forEach(element => {
                    this.base.push({
                        id:element.id,
                        name:element.name
                    })
        ...
        methods: {
            storing(){
                store.dispatch('base/store',this.form).then((data)=>{
                    this.base.push({
                        id:data.data.base.id,
