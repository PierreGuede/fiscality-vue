    ...
    <div class="p-2">
        ...
        <div class="flex">
            <div class="w-9/12 self-center">
                <h1 class="text-xl font-bold">Les categories d'activités</h1>
            </div>
            <div class="w-3/12 self-center mx-auto">
                ...
                <div v-if="modal" class="fixed inset-0  w-full h-full bg-opacity-30 bg-gray-200 " >
                    <div class=" bg-white p-4 z-100 w-1/2 mx-auto mt-24 lg:my-8 self-center " >
                        <div class="flex ">
                            <div class="w-11/12">
                                <h1 class="text-xl font-medium">Ajout d'une categorie</h1>
                            </div>
                        ...
                        <div>
                            <form action="" @submit.prevent="storing">
                                <div class="mt-4">
                                    <InputView  name="name" labeling="Nom de la category" v-model="form.name" typeInput="text" idInput="name" />
                                </div>
        ...
        <ListCategoryView :listCategory="category"/>
    </div>
</template>
<script>
    import store from "@/store/index"
    import InputView  from '@/components/InputView.vue'
import ListCategoryView from "@/components/CategoryComponents/ListCategoryView.vue"
    ...
    export default{
        ...
        mounted() {
            store.dispatch('category/index',this.$route.params.id).then((data)=>{
        ...
        methods: {
            storing(){
                store.dispatch('category/store',this.form).then((data)=>{
                    this.category.push({
