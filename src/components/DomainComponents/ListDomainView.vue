<template>
    <div class="p-2 mt-4 space-y-4">
        ...
        <table class="space-y-4 p-2 w-full text-sm text-left text-gray-500  ">
            ...
            <tbody class=" space-y-4 p-2" v-for="listDomain in listDomain" :key="listDomain.id">
                <tr class="mt-2 border-b transition duration-200 hover:shadow-lg">
                    ...
                    <th class="flex space-x-4 px-6 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap">
                        <router-link :to="{name:'DomainInfoView',params:{id:listDomain.id}}" class="text-blue-500">Voir plus</router-link>
                        <span class="text-red-500" @click="loadeleting(listDomain.id)">Supprimer</span>
                    </th>
                </tr>
...
export default{
    ...
    methods: {
        ...
        deleting(id){
           store.dispatch('domain/delete',id).then(()=>{
                this.$router.go()
           })
        }
    },
